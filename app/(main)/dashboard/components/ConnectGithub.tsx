...
function StatusBadge({ status }: { status: SyncStatus }) {
    ...
    return (
        ...
        >
            <span
                className={`size-1.5 rounded-full ${config.dotColor} ${status === "SYNCING" ? "animate-pulse" : ""
                    }`}
                aria-hidden="true"
            />
            {config.label}
...
function ErrorAlert({ message }: { message: string }) {
    return (
        <div
            className="mt-3 rounded-lg bg-red-500/10 px-3 py-2 text-sm text-red-500"
            role="alert"
        >
            {message}
        </div>
...
export default function ConnectGithub({
    ...
    const isDisabled = loading || syncStatus === "SYNCING";
        ...
        <div className="border bg-card p-5 sm:p-6">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <Button
                    ...
                    onClick={handleSync}
                    className="shrink-0"
                    aria-label="Sync GitHub repositories"
                >
